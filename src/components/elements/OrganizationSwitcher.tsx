  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button disabled={loading} variant="ghost" className="w-[180px] justify-between h-9 px-2 hover:bg-accent/50 group">
          <div className="flex items-center gap-2 overflow-hidden">
            <div className="h-6 w-6 rounded bg-primary text-primary-foreground flex items-center justify-center text-[10px] font-bold shrink-0 shadow-sm group-hover:scale-105 transition-transform">
              {loading ? <Loader2 className="h-3 w-3 animate-spin" /> : (currentOrg?.name?.charAt(0) || <Home className="h-3 w-3" />)}
            </div>
            <div className="text-left overflow-hidden">
              <p className="text-[11px] font-bold truncate leading-tight">{currentOrg?.name || t('org.select_title')}</p>
