...
function NotesIndex() {
  ...
  const folders = notes.filter((n: any) => n.isFolder)
  const pages = notes.filter((n: any) => !n.isFolder)
      ...
      <div className="max-w-6xl mx-auto p-8 md:p-12 space-y-12 animate-reveal">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
           <div className="space-y-2">
              <div className="flex items-center gap-2 text-primary">
                 ...
                 <span className="text-xs font-black uppercase tracking-widest">{t('notes.title')}</span>
              </div>
              <h2 className="text-4xl font-black tracking-tight">{t('notes.welcome_title')}</h2>
              <p className="text-muted-foreground font-medium max-w-md">{t('notes.welcome_desc')}</p>
           </div>
           <Button 
             size="lg" 
        ...
        {notes.length > 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {folders.map((folder: any) => (
                    <Card 
                      ...
                      onClick={() => navigate({ to: `/homes/${orgId}/notes` })}
                    >
                      <CardContent className="p-4 flex items-center gap-4">
                        ...
                        <div className="flex-1 truncate">
                           <p className="font-bold text-sm truncate">{folder.title || t('notes.untitled_folder')}</p>
                           <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">{t('notes.empty_folder')}</p>
                        </div>
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                 {pages.map((page: any) => (
                    <Card 
                      ...
                      onClick={() => navigate({ to: `/homes/${orgId}/notes/${page.id}` })}
                    >
          ...
          <div className="py-20 flex flex-col items-center justify-center text-center">
             ...
             <h3 className="text-xl font-bold mb-2">{t('notes.no_notes')}</h3>
